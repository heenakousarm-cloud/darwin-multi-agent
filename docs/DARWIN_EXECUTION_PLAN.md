...
Darwin powers the **PMF Acceleration Engine** — an AI Product Copilot that shortens the path to Product-Market Fit by:
...
|-------|----------|----------------|---------|
| **Watcher** | 🕵️ Eyes | Detect friction signals from analytics | Cron / Manual |
| **Analyst** | 🧠 Brain | Diagnose root cause, propose fix | New Signal in DB |
| **Engineer** | 👩‍💻 Hands | Write code, create PR | Human Approval |
...
│                                                                              │
│   ┌────────────────────────────────────────────────────────────────────┐    │
│   │              LAYER 1: DARWIN AGENTS (Python Backend)                │    │
│   │                                                                     │    │
│   │   ┌─────────────┐    ┌─────────────┐    ┌─────────────┐            │    │
...
│   │   ┌─────────────────────────────────────────────────────────┐      │    │
│   │   │                     MongoDB Atlas                        │      │    │
│   │   │  • signals collection    • insights collection          │      │    │
│   │   │  • ux_issues collection  • experiments collection       │      │    │
│   │   └─────────────────────────────────────────────────────────┘      │    │
│   │          │                                                          │    │
│   │          │ PostHog API                              GitHub API      │    │
│   │          ▼                                          ▲               │    │
│   │   ┌─────────────┐                           ┌──────┴──────┐        │    │
│   │   │   PostHog   │                           │   GitHub    │        │    │
│   │   │  Analytics  │                           │  Pull Req   │        │    │
...
│                                  ▼                                           │
│   ┌────────────────────────────────────────────────────────────────────┐    │
│   │              LAYER 2: NITROSTACK MCP SERVER (TypeScript)            │    │
│   │                                                                     │    │
│   │   ┌─────────────────────────────────────────────────────────┐      │    │
│   │   │                     @Resources                           │      │    │
│   │   │  • product_health    • signals_alerts   • ux_issues     │      │    │
│   │   │  • decision_center   • experiments      • pmf_audit     │      │    │
│   │   └─────────────────────────────────────────────────────────┘      │    │
│   │                                                                     │    │
│   │   ┌─────────────────────────────────────────────────────────┐      │    │
│   │   │                       @Tools                             │      │    │
│   │   │  • investigate_signal    • approve_recommendation       │      │    │
│   │   │  • create_task           • add_to_roadmap               │      │    │
│   │   │  • view_experiment       • trigger_engineer             │      │    │
│   │   └─────────────────────────────────────────────────────────┘      │    │
│   │                                                                     │    │
│   │   ┌─────────────────────────────────────────────────────────┐      │    │
│   │   │                      @Widgets                            │      │    │
│   │   │  • DashboardWidget       • SignalCard                   │      │    │
│   │   │  • InsightCard           • ExperimentCard               │      │    │
│   │   │  • MetricCard            • RecommendationCard           │      │    │
│   │   └─────────────────────────────────────────────────────────┘      │    │
│   └────────────────────────────────────────────────────────────────────┘    │
│                                  │                                           │
│                                  │ Deploy                                    │
│                                  ▼                                           │
│   ┌────────────────────────────────────────────────────────────────────┐    │
│   │              LAYER 3: NITROCLOUD + NITROCHAT (Deployment)           │    │
│   │                                                                     │    │
│   │   • One-click deployment from GitHub                               │    │
│   │   • Auto-scaling managed infrastructure                            │    │
│   │   • Shareable demo link via NitroChat                              │    │
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   PostHog ──► Watcher Agent ──► MongoDB (signals)                       │
│                                      │                                   │
│                                      ▼                                   │
│                              Analyst Agent                               │
│                                      │                                   │
│                                      ▼                                   │
│                         MongoDB (insights: PENDING)                      │
│                                      │                                   │
│                                      ▼                                   │
│   ┌──────────────────────────────────────────────────────────────────┐  │
│   │                 NITROSTACK DECISION CENTER                        │  │
│   │                                                                   │  │
│   │   @Resource('decision_center') ──► Renders Figma-style cards     │  │
│   │                                                                   │  │
│   │   User clicks "Add to Roadmap" or "Create Task"                  │  │
│   │                     │                                             │  │
│   │                     ▼                                             │  │
│   │   @Tool('approve_recommendation') ──► Updates MongoDB            │  │
│   └──────────────────────────────────────────────────────────────────┘  │
│                                      │                                   │
│                                      ▼                                   │
│                         MongoDB (insights: APPROVED)                     │
│                                      │                                   │
│                                      ▼                                   │
│                            Engineer Agent                                │
│                                      │                                   │
...
│  [Logo]                     │
├─────────────────────────────┤
│  ✓ Product Health           │  ← Selected (white bg, rounded)
│  ○ PMF Audit                │
│  ⚠ Signals & Alerts    [7]  │  ← Badge for count
│  ◎ Decision Center    [12]  │
│  △ Experiments         [3]  │
│  ↔ Infra Correlation        │
│  ✋ UX Intelligence     [5]  │
...
```
┌─────────────────────────────────────────────────────────────────┐
│  ⚠ Significant Drop-off in Onboarding Flow          [Investigate]│
│                                                                  │
│  Step 3 completion rate dropped from 68% to 42% in the last     │
│  48 hours.                                                       │
│                                                                  │
│  Metric: Activation Rate    Change: -26%    Confidence: 96%     │
│  ○ 2 hours ago                                                   │
└─────────────────────────────────────────────────────────────────┘
...
```
┌─────────────────────────────────────────────────────────────────┐
│  Add SSO Integration                      [Critical]             │
│                                                    [Add to Roadmap →]│
│  Blocking $180K+ in enterprise deals. 67% of enterprise         │
│  prospects require SSO.                                          │
│                                                                  │
│  Impact: +$180K pipeline, +15% close rate                       │
│  ○ 3 weeks                          Confidence: 98%             │
└─────────────────────────────────────────────────────────────────┘

- Priority badge: Critical (red), High (yellow), Medium (gray)
- Action button: "Add to Roadmap →" - dark bg, white text
...
```
┌─────────────────────────────────────────────────────────────────┐
│  ✋ Payment Form Input Not Responding              [critical]    │
│                                                                  │
│  Page: Checkout - Step 2  •  Affected: 234 users  •  Replays: 87│
│                                                                  │
│  Users rage-clicking on credit card input. Field appears        │
│  disabled due to CSS z-index issue.                              │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  IMPACT                        CONFIDENCE               │    │
│  │  Blocking 12% of checkout      98%                      │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  RECOMMENDED FIX                                        │    │
│  │  Fix CSS z-index - payment iframe      [Create Task →]  │    │
│  │  covered by form overlay                                │    │
│  └─────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────┘
...
│                                                                       │
│  Product Health                                                       │
│  Real-time insights into your product metrics                         │
│                                                                       │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  PMF Readiness Score                              ↗ +8 pts     │  │
│  │                                                                 │  │
│  │  67 /100                                                        │  │
│  │  ████████████████████████████░░░░░░░░░░                        │  │
│  │                                                                 │  │
│  │  Approaching PMF - Focus on retention & activation              │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                                                                       │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐                │
│  │ Active   │ │Activation│ │ 4-Week   │ │   MRR    │                │
│  │ Users    │ │ Rate     │ │Retention │ │          │                │
│  │ 3,247    │ │ 72%      │ │ 45%      │ │ $24.3K   │                │
│  │ ↗+12.3%  │ │ ↗+5.2%   │ │ ↘-3.1%   │ │ ↗+18.7%  │                │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘                │
│                                                                       │
│  ┌─────────────────────┐  ┌─────────────────────┐                    │
│  │  Cohort Retention   │  │  Activation Trend   │                    │
│  │  [Chart: W1-W6]     │  │  [Chart: Mon-Sun]   │                    │
│  └─────────────────────┘  └─────────────────────┘                    │
│                                                                       │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  Feature Adoption                                               │  │
...
│                                                                       │
│  Signals & Alerts                                                     │
│  AI-detected anomalies and opportunities                              │
│                                                                       │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐                │
│  │ Critical │ │Opportun- │ │  Total   │ │   Avg    │                │
│  │ Alerts   │ │ ities    │ │ Signals  │ │Confidence│                │
│  │    1     │ │    2     │ │    5     │ │   90%    │                │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘                │
│                                                                       │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │ ⚠ Significant Drop-off in Onboarding Flow       [Investigate] │  │
│  │   Step 3 completion rate dropped from 68% to 42%...           │  │
│  │   Metric: Activation Rate  Change: -26%  Confidence: 96%      │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                                                                       │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │ ↗ Power User Segment Growing Rapidly            [Investigate] │  │
│  │   Users completing 5+ actions/day increased 34%...            │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                                                                       │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │ ⚠ API Integration Completion Declining          [Investigate] │  │
│  │   Only 23% of new users completing API setup...               │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                                                                       │
└──────────────────────────────────────────────────────────────────────┘
```
...
│  │ Recomm.  │ │ Priority │ │ Priority │ │Confidence│                │
│  │    5     │ │    2     │ │    2     │ │   91%    │                │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘                │
│                                                                       │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  Add SSO Integration                 [Critical]                 │  │
│  │  Blocking $180K+ in enterprise deals...       [Add to Roadmap →]│  │
│  │  Impact: +$180K pipeline  ○ 3 weeks  Confidence: 98%           │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                                                                       │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  Fix Onboarding Step 3 Drop-off      [Critical]                 │  │
│  │  Activation rate dropped 26% in 48 hours...   [Add to Roadmap →]│  │
│  │  Impact: +26% activation recovery  ○ 3 days  Confidence: 96%   │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                                                                       │
└──────────────────────────────────────────────────────────────────────┘
```
...
│                                                                       │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐                │
│  │ Critical │ │   High   │ │ Sessions │ │  Rage    │                │
│  │ Issues   │ │ Priority │ │ Analyzed │ │  Clicks  │                │
│  │    1     │ │    2     │ │  15.6K   │ │   234    │                │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘                │
│                                                                       │
│  ┌────────────────────────────────────────────────────────────────┐  │
...
├── DARWIN_EXECUTION_PLAN.md          # This file
├── .env.example                      # Environment template
├── .gitignore
├── docker-compose.yml                # Local MongoDB (optional)
│
├── agents/                           # LAYER 1: Python Agents
│   ├── __init__.py
│   ├── config.py                     # Shared configuration
│   ├── db.py                         # MongoDB connection
│   ├── models.py                     # Pydantic models (Signal, Insight, etc.)
│   ├── watcher.py                    # Agent 1: The Eyes 🕵️
│   ├── analyst.py                    # Agent 2: The Brain 🧠
│   └── engineer.py                   # Agent 3: The Hands 👩‍💻
│
├── nitrostack/                       # LAYER 2: NitroStack MCP Server
│   ├── package.json
│   ├── tsconfig.json
...
│   │   │   ├── productHealth.ts      # PMF score, metrics
│   │   │   ├── signalsAlerts.ts      # Watcher output
│   │   │   ├── uxIntelligence.ts     # Analyst rage click analysis
│   │   │   ├── decisionCenter.ts     # Pending recommendations
│   │   │   ├── experiments.ts        # A/B tests
│   │   │   └── pmfAudit.ts           # Feature audit
│   │   │
│   │   ├── tools/                    # @Tool decorators
│   │   │   ├── investigate.ts        # "Investigate" button
│   │   │   ├── createTask.ts         # "Create Task" → triggers Engineer
│   │   │   ├── addToRoadmap.ts       # "Add to Roadmap" button
│   │   │   └── viewDetails.ts        # "View Details" button
│   │   │
│   │   ├── widgets/                  # @Widget decorators
│   │   │   ├── MetricCard.tsx        # Reusable metric display
│   │   │   ├── SignalCard.tsx        # Signal/Alert card
│   │   │   ├── InsightCard.tsx       # UX Intelligence issue card
│   │   │   ├── RecommendationCard.tsx # Decision Center card
│   │   │   └── ExperimentCard.tsx    # Experiment progress card
│   │   │
│   │   └── db/
│   │       └── mongodb.ts            # MongoDB client for NitroStack
│   │
│   └── styles/
│       └── darwin.css                # Figma design tokens as CSS
│
├── demo-app/                         # Demo e-commerce app (existing)
│   ├── package.json
│   ├── src/
│   │   ├── App.tsx
│   │   └── components/
│   │       └── PaymentForm.tsx       # Contains the z-index bug!
│   └── routes.json                   # URL → Component mapping
│
├── scripts/
│   ├── seed_data.py                  # Seed MongoDB with demo data
│   ├── run_watcher.py                # Manual trigger for Watcher
│   ├── run_analyst.py                # Manual trigger for Analyst
│   ├── run_engineer.py               # Manual trigger for Engineer
│   └── demo_flow.py                  # Run complete demo sequence
│
├── data/
│   └── mock/
│       ├── signals.json              # Sample signals
│       ├── insights.json             # Sample insights
│       └── experiments.json          # Sample experiments
│
└── tests/
    ├── test_watcher.py
    ├── test_analyst.py
...
OUTPUT: Signal document in MongoDB → Displayed in "Signals & Alerts" page
```
...
- [ ] `agents/db.py` — MongoDB helper
- [ ] `agents/watcher.py` — Watcher Agent
- [ ] `scripts/seed_data.py` — Mock data seeder
...
PROCESS:
  1. Map DOM element → React component file
...
OUTPUT: UX Issue document in MongoDB → Displayed in "UX Intelligence" page
```
...
| 8.4.2 | `demo-app/routes.json` | URL → Component mapping |
...
- [ ] `demo-app/routes.json` — URL mapping
...
NitroStack provides:
- `@Resource` — Read data from MongoDB, return to LLM/UI
- `@Tool` — Execute actions (approve, create task, etc.)
- `@Widget` — Render beautiful UI components
...
export class InvestigateTool {
  ...
  async execute({ signal_id }) {
    ...
    return {
      success: true,
      message: `🔍 Investigation started for signal ${signal_id}. Analyst Agent will analyze shortly.`
    };
  }
}
```
...
export class CreateTaskTool {
  ...
  async execute({ issue_id, approved_by }) {
    ...
    return {
      success: true,
      message: `✅ Task created! Engineer Agent will now generate the code fix and open a PR.`,
...
export class AddToRoadmapTool {
  ...
  async execute({ recommendation_id, approved_by }) {
    ...
    return {
      success: true,
      message: `📋 Added to roadmap! This recommendation is now prioritized.`
    };
  }
}
```
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-6">
      ...
      <div className="text-sm text-gray-500 mt-3">
        Page: {issue.page_info} • Affected: {issue.affected_users} users • Replays: {issue.replays_count}
      </div>
...
class EngineerAgent:
    ...
    def create_pull_request(self, issue, new_code):
        ...
        pr = repo.create_pull(
            title=f"🧬 Darwin Fix: {issue['title']}",
            body=f"""
## 🕵️ Issue Detected
  ...
  return (
    <div className="payment-container">
      ...
      <iframe 
        ...
        style={{ zIndex: 1 }}  // ← THE BUG: Should be higher than overlay
      />
...
1. OPEN NITROCHAT / NITROSTUDIO
   → Show the PMF Acceleration Engine dashboard
   → "This is our AI Product Copilot..."

2. NAVIGATE TO UX INTELLIGENCE
   → Show the stats: 1 Critical Issue, 234 Rage Clicks
   → "Darwin has detected a critical UX issue..."

3. SHOW THE ISSUE CARD
   → "Payment Form Input Not Responding"
   → Point out: affected users, confidence, recommended fix
   → "Darwin analyzed session recordings and found the root cause..."

4. CLICK "CREATE TASK" 
   → "As a PM, I agree with this fix. Let's do it."
   → Click the button
   → Show success message

5. WATCH TERMINAL (split screen)
   → Run: python scripts/run_engineer.py
   → "Darwin is now writing the code..."
...
6. THE MAGICAL MOMENT 🎉
   ...
   → Refresh
   → BOOM — PR appears!
   → "🧬 Darwin Fix: Payment Form Input Not Responding"
   → Show the beautiful PR description

7. SHOW THE FIX
   → Open the PR diff
   → Show: z-index changed from 1 to 101
   → "A one-line fix that was blocking 12% of checkouts!"
```
...
> 
> We call this the 'Insight-to-Action Latency' — and it's killing your PMF velocity."

### Slide 2: Introducing Darwin (30 seconds)

> "We built Darwin — an Autonomous Growth Engineer.
> 
> Darwin doesn't just REPORT the weather. It FIXES the roof.
> 
> It monitors your analytics, detects friction, analyzes root causes, and opens Pull Requests — all automatically."
...
6. **Switch to GitHub → BOOM — PR appears!** 🎉
...
> 
> This is the future of product development — AI that doesn't just analyze, but acts."
...
```
UX Intelligence → "Create Task" → Engineer Agent → GitHub PR
```

This single flow demonstrates:
- ✅ AI detecting issues (Watcher output)
- ✅ AI analyzing root cause (Analyst output)
- ✅ Human-in-the-loop approval (NitroStack Tool)
- ✅ AI executing fixes (Engineer Agent)
- ✅ Real code changes (GitHub PR)
...
*Version: 2.0 — NitroStack + Figma Integration*
